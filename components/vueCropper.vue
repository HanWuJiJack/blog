<template>
  <!-- vueCropper 剪裁图片实现-->
  <el-dialog title="图片剪裁" :visible.sync="dialogVisible" :fullscreen="true" v-if="dialogVisible" append-to-body>
    <div class="cropper-content">
      <div class="cropper" style="text-align:center">
        <vueCropper ref="cropper" :img="option.img" :output-size="option.size" :output-type="option.outputType"
          ...
          :fixed-box="option.fixedBox"></vueCropper>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    changeImage(file) {
      this.loading = false
      this.option.fixedNumber = file.fixedNumber
      this.option.fixed = file.fixed
